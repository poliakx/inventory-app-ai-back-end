...
- ~~products: deleting a product referenced by stock movements returned 500 "Internal server error" instead of a friendly 409~~ — fixed 2026-09-16: the code checked `error.code === "23503"` (foreign_key_violation), but `ON DELETE RESTRICT` (used on both `stock_movements.product_id` and `recipe_ingredients.product_id`) actually raises `23001` (restrict_violation) — a different SQLSTATE for the same "can't delete, still referenced" case. Centralized handling of both codes in `error.middleware.js` via a `CONSTRAINT_MESSAGES` lookup keyed by `err.constraint`, replacing the ad-hoc `try/catch` in `products.service.js` — any future `ON DELETE RESTRICT` violation across any resource now gets a friendly message for free. Regression test added in `product.test.js`.
